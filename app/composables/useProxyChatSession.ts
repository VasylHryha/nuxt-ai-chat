...
interface UseProxyChatSessionOptions {
  onFirstChatCreated?: (id: string) => void
}

export function useProxyChatSession(options: UseProxyChatSessionOptions = {}) {
  const auth = useAuth()
  const { createChat, saveMessages, loadChat, generateTitle } = useChatPersistence()

  const provider = ref<'openrouter' | 'anthropic' | 'google'>('openrouter')
  const model = ref('deepseek/deepseek-r1')
  ...
  async function loadExistingChatById(chatId: string) {
    ...
    try {
      ...
      provider.value = response.chat.provider as any
  ...
  async function ensureChatId(firstUserText: string) {
    ...
    const id = await createChat(provider.value, model.value, title, 'proxy')
    if (!id)
  ...
  async function sendMessage(payload: { text: string }) {
    ...
    try {
      ...
      // Stream from proxy endpoint for the current provider
      ...
      const streamResp = await fetch(`/api/v1/${provider.value}/chat.stream`, {
        method: 'POST',
        headers: {
      ...
      if (!streamResp.ok || !streamResp.body) {
        // Proxy providers don't have non-streaming fallback
        throw new Error(`Streaming failed for ${provider.value} (status: ${streamResp.status})`)
      }
      ...
      console.error('[Proxy Chat] Send failed:', err)
