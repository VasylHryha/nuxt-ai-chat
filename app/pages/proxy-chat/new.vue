...
const { chat, currentChatId, provider, model } = useAiChatSession({
  onFirstChatCreated: (id) => router.replace(getChatRouteFor('openrouter', model.value, id, 'proxy')),
})
  ...
  <div class="flex flex-col h-[calc(100vh-12rem)] max-h-[800px]">
    <div class="flex items-center justify-between mb-4 pb-4 border-b border-[var(--panel-border)]">
      <div class="flex items-center gap-3">
        ...
        <div>
          <h1 class="text-2xl font-semibold text-fg">Proxy Chat</h1>
          <p class="text-sm text-fg-muted">Start a new proxy conversation</p>
        </div>
      </div>
    ...
    <div class="flex-1 overflow-y-auto space-y-4 mb-4 pr-2">
      ...
      <div v-for="(m, index) in chat.messages" :key="m.id || index" class="flex items-start gap-3"
